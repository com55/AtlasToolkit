...
const REGION_CORE_KEYS = new Set(['index', 'rotate', 'bounds', 'xy', 'size', 'offsets', 'orig', 'offset', 'split', 'pad']);

function _isPageLine(line) {
  return /^.+\.png$/i.test(line.trim());
}
...
export function convertAtlasToNewFormat(atlasText) {
  ...
  function flushRegion() {
    ...
    for (const [k, values] of Object.entries(regionKV)) {
      if (!REGION_CORE_KEYS.has(k)) result.push(`  ${k}: ${values.join(', ')}`);
    }
