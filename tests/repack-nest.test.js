...
// 2x3 (w=2,h=3) asymmetric pattern -- catches a transpose-direction mistake
// that a symmetric fixture would silently pass.
//   col: 0 1
// row0:  1 0
// row1:  0 0
// row2:  0 1
const SRC_W = 2, SRC_H = 3;
const SRC = new Uint8Array([1, 0, 0, 0, 0, 1]);
...
test('rotate90CW: 2x3 -> 3x2, matches hand-rotated expectation', () => {
  ...
  // row0:  0 0 1
  // row1:  1 0 0
  assert.deepEqual(Array.from(out), [0, 0, 1, 1, 0, 0]);
});
...
test('rotate90CCW: 2x3 -> 3x2, matches hand-rotated expectation', () => {
  ...
  // row0:  0 0 1
  // row1:  1 0 0
  // (CCW of this particular asymmetric pattern happens to differ from CW --
  // verified by hand: CCW maps new(x,y) = old(w-1-y, x))
  assert.deepEqual(Array.from(out), [0, 0, 1, 1, 0, 0]);
});

test('rotate180: 2x3 -> 2x3, reversed', () => {
  const out = rotate180(SRC, SRC_W, SRC_H);
  assert.deepEqual(Array.from(out), [1, 0, 0, 0, 0, 1]);
});
